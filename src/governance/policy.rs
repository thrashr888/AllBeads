    ...
    fn default_description(policy_type: &PolicyType) -> String {
        match policy_type {
            PolicyType::RequireDescription => {
                "All beads must have a description".to_string()
            }
            PolicyType::MaxInProgress { max_count } => {
                format!("Limit concurrent in-progress beads to {} per assignee", max_count)
            }
            ...
            PolicyType::RequirePriority => {
                "All beads must have a valid priority set".to_string()
            }
            PolicyType::RequireAssignee => {
                "Open beads should have an assignee".to_string()
            }
...
pub fn default_policies() -> Vec<Policy> {
    vec![
        ...
        Policy::new("max-in-progress", PolicyType::MaxInProgress { max_count: 3 }),
